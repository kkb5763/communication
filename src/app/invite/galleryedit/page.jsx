...
export default function GalleryPage() {
  ...
  const fetchImages = async () => {
    const { data, error } = await supabase.storage.from('wedding-gallery').list('', {
      limit: 100,
      offset: 0,
    ...
    const urls = (data || [])
      ...
      .map((file) => supabase.storage.from('wedding-gallery').getPublicUrl(file.name).data.publicUrl);
  ...
  return (
    <div className="min-h-screen bg-[#fffdf7] text-center px-4 py-10">
      ...
      <div className="flex justify-center gap-4">
        ...
        <label className="bg-[#f0f0f0] hover:bg-[#e4e4e4] cursor-pointer text-[#3e312b] font-medium py-2 px-6 rounded-full shadow">
          📤 이미지 업로드
